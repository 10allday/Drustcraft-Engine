...
drustcraftw_regenerate:
    ...
    events:
        on drustcraft load:
            ...
            - if <[create_tables]>:
                - ~sql id:drustcraft_database 'update:INSERT INTO `drustcraft_version` (`name`,`version`) VALUES ("drustcraft_regenerate",'1');'
        ...
        on player places block server_flagged:drustcraft_regenerate:
            - if <yaml.list.contains[drustcraft_server]> && <player.gamemode> == SURVIVAL && <server.flag[drustcraft_regenerate_blocks].contains[<context.material>]>:
                - sql id:drustcraft_server 'update:DELETE FROM `<server.flag[drustcraft_database_table_prefix]>drustcraft_regenerate` WHERE server="<bungee.server>" AND action="movement" AND world=""<context.location.world.name>" AND x=<context.location.x.round> AND y=<context.location.y.round> AND z=<context.location.z.round>;'
                - sql id:drustcraft_server 'update:INSERT INTO `<server.flag[drustcraft_database_table_prefix]>drustcraft_regenerate` (`server`, `action`, `world`, `x`, `y`, `z`, `material`, `date`) VALUES ("<bungee.server>", "place", "<context.location.world.name>", <context.location.x.round>, <context.location.y.round>, <context.location.z.round>, "<context.material>", <util.time_now.epoch_millis.div[1000].round>);'

        on player breaks block server_flagged:drustcraft_regenerate:
            - if <yaml.list.contains[drustcraft_server]> && <player.gamemode> == SURVIVAL && <server.flag[drustcraft_regenerate_blocks].contains[<context.material>]>:
                - sql id:drustcraft_server 'update:DELETE FROM `<server.flag[drustcraft_database_table_prefix]>drustcraft_regenerate` WHERE server="<bungee.server>" AND action="place" AND world=""<context.location.world.name>" AND x=<context.location.x.round> AND y=<context.location.y.round> AND z=<context.location.z.round>;'
                ...
                - if <entry[sql_result].result.size||0> == 0:
                    - define action:break
                - sql id:drustcraft_server 'update:INSERT INTO `<server.flag[drustcraft_database_table_prefix]>drustcraft_regenerate` (`server`, `action`, `world`, `x`, `y`, `z`, `material`, `date`) VALUES ("<bungee.server>", "<[action]>", "<context.location.world.name>", <context.location.x.round>, <context.location.y.round>, <context.location.z.round>, "<context.material>", <util.time_now.epoch_millis.div[1000].round>);'
...
drustcraftt_regenerate:
    ...
    script:
        ...
        - define decay_time:<[now]<yaml[drustcraft_server].read[drustcraft.regenerate.decay_delay]>]>
        ...
        - foreach <entry[sql_result].result>:
            ...
            - if <util.random.decimal[0].to[1]> <= <[chance]>:
                ...
                - if <[action]> != "break" OR <location[<[x]>,<[y]>,<[z]>,<[world]>].find.blocks[].within[1].parse[material.name].deduplicate.exclude[air].size||0> > 1:
