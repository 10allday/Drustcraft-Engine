...
drustcraftw_region:
  ...
  events:
    ...
    on player enters polygon:
      - run drustcraftt_region_enter def:<context.area.note_name>

    on player enters cuboid:
      - run drustcraftt_region_enter def:<context.area.note_name>

    on player exits polygon:
      - run drustcraftt_region_exit def:<context.area.note_name>
    ...
    - if <[note_name].starts_with[drustcraft_region_]>:
      ...
      - if <[type]> != null:
        - if <server.has_flag[drustcraft.region.type.<[type]>]>:
          - run <server.flag[drustcraft.region.type.<[type]>]> def:enter|<[world]>|<[region]>|<[type]>|<[title]>
      - else if <[title]> != null:
        - title title:<&f><[title]>
...
drustcraftt_region_exit:
  ...
  script:
    - if <[note_name].starts_with[drustcraft_region_]>:
      ...
      - if <[type]> != null:
        - if <server.has_flag[drustcraft.region.type.<[type]>]>:
          - run <server.flag[drustcraft.region.type.<[type]>]> def:exit|<[world]>|<[region]>|<[type]>|<[title]>
